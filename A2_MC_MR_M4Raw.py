...
initx = torch.tensor(initx).to(device)
inity = torch.tensor(inity).to(device)
...
class BasicBlock(torch.nn.Module):
    ...
    def forward(self, x, fft_forback, k, c, batch_size):
        ...
        if torch.is_complex(k) == False: 
            k = torch.view_as_complex(k)
        if torch.is_complex(c) == False:
...
if (platform.system() =="Windows"):  #####platform.system()获取操作系统类型
    rand_loader = DataLoader(dataset=data_set(data_path, training), batch_size=batch_size, num_workers=0,
                             shuffle=False)
else:
    rand_loader = DataLoader(dataset=data_set(data_path, training), batch_size=batch_size, num_workers=4,
                             shuffle=False)

optimizer = torch.optim.Adam([{'params':model.module.fft_forback.parameters(), 'lr':0.003},
                              {'params':model.module.fcs.parameters(), 'lr':learning_rate}
                          ])

model_dir = "./%s/MRI_MC_MR_Net_layer_%d_group_%d_cs_ratio_%d_%s" % (args.model_dir, layer_num, group_num, cs_ratio, mask_type)

log_file_name = "./%s/Log_MRI_MC_MR_Net_layer_%d_group_%d_cs_ratio_%d_%s.txt" % (args.log_dir, layer_num, group_num, cs_ratio, mask_type)
...
for epoch_i in range(start_epoch+1, end_epoch+1):
    for data in rand_loader:
        coil_map, kspace, ground_truth, seg_label_ori = data
        seg_label = seg_label_ori[:, :, 0, :, :]
        ## whole
        seg_label_cordinate = np.isin(seg_label, np.arange(1, 117))

        seg_labels = seg_label_cordinate.astype(np.int32)
        # ##### subcortical
        # seg_labels = np.where(
        ...
        loss_discrepancy = torch.mean(torch.pow(torch.abs(x_output - ground_truth), 2))

        seg_discrepancyloss = torch.mean(torch.pow(torch.abs(x_output * seg_pred[-1] - ground_truth * seg_labels), 2))

        discrepancy_segloss = torch.mean(torch.pow(torch.abs(x_output * seg_labels - ground_truth * seg_pred[-1]), 2))
        segm_loss = seg_loss(seg_pred[0], seg_labels, 1)
        for k in range(layer_num - 1):
            segm_loss += seg_loss(seg_pred[k + 1], seg_labels, 1)
        segm_loss = segm_loss / layer_num
        ...
        lamda = torch.Tensor([0.01]).to(device)
        # loss_all = loss_discrepancy
        loss_all = loss_discrepancy + torch.mul(gamma, loss_constraint) + torch.mul(0.1, seg_discrepancyloss)\
                   + torch.mul(0.1, segm_loss)

        # Zero gradients, perform a backward pass, and update the weights.
        optimizer.zero_grad()
        loss_all.backward()
        optimizer.step()
        model.module.fft_forback.inity.data = torch.clamp(model.module.fft_forback.inity.data, 0, 1)

        output_data = "[%02d/%02d] Total Loss: %.5f, Discrepancy Loss: %.5f,  Constraint Loss: %.5f, Dice loss: %.5f " \
                      " seg_discrepancy Loss: %.5f,   discrepancy_seg Loss: %.5f,\n" % (epoch_i, end_epoch,
                      loss_all.item(), loss_discrepancy.item(), loss_constraint, segm_loss, seg_discrepancyloss,
                      discrepancy_segloss)
